...
def benchmark_triton_folder(folder_name, audio_array, num_warmup=1, num_runs=3):
    ...
    print(f"Benchmarking ({num_runs} runs)...")
    times = []
    peak_memories = []  # --- [唯一新增]: 显存峰值记录列表 ---

    for i in range(num_runs):
        if torch.cuda.is_available():
            torch.cuda.synchronize()
            torch.cuda.reset_peak_memory_stats(device)  # --- [唯一新增]: 重置显存峰值 ---
        ...
        if torch.cuda.is_available():
            torch.cuda.synchronize()
            peak_mem_mb = torch.cuda.max_memory_allocated(device) / (1024 * 1024)  # --- [唯一新增]: 获取显存峰值 ---
        ...
        mem_str = f" | Peak VRAM: {peak_mem_mb:.1f} MB" if torch.cuda.is_available() else ""  # --- [唯一新增]: 打印格式 ---
    ...
    return {
        ...
        'transcription': transcription,
        'tokens': tokens,
        'mean_mem_mb': np.mean(peak_memories) if peak_memories else 0  # --- [唯一新增]: 返回显存数据 ---
    }
...
def main():
    ...
    try:
        ...
        # --- [唯一新增]: 打印显存 ---
        if 'mean_mem_mb' in results and results['mean_mem_mb'] > 0:
            print(f"Peak VRAM: {results['mean_mem_mb']:.1f} MB")
